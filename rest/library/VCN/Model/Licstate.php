 ...
 * Certifications
 */
class VCN_Model_Licstate extends VCN_Model_Base_Licstate
{
	protected $valid = array('STFIPS','STATE','STATENAME','DATASRCE','INCLUDE','DLU');
	
	public function countLicstates($params, $fields=false) 
	{
 ...
 // echo $stmt->getSqlQuery();exit;		
	...
	public function listLicstates($params, $fields=false) 
	{
		$query = $this->parseParams($params, $this->valid);
        $stmt  = $this->getTable()->createQuery();
		$select= $this->parseFields($stmt, $fields);
	...
	public function getLicstate($params, $fields=false) 
	{
 		$query = $this->parseParams($params, $this->valid);
 		$stmt  = $this->getTable()->createQuery();
  		$required = array('stfips');
