 ...
 * Certifications
 */
class VCN_Model_Licauth extends VCN_Model_Base_Licauth
{
	protected $valid = array( 'STFIPS','AREATYPE','AREA','LICAUTHID','NAME1','NAME2','NAME3','ADDRESS1','ADDRESS2','CITY','ST','ZIP','ZIPEXT','TELEPHONE','TELEEXT','FAX','CONTACT','EMAIL','URL');

	public function countLicauths($params, $fields=false) 
	{
	...
	public function listLicauths($params, $fields=false) 
	{
		$query = $this->parseParams($params, $this->valid);
        $stmt  = $this->getTable()->createQuery();
		$select= $this->parseFields($stmt, $fields);
	...
	public function getLicauth($params, $fields=false) 
	{
 		$query = $this->parseParams($params, $this->valid);
 		$stmt  = $this->getTable()->createQuery();
  		$required = array('stfips','licauthid');
