 ...
 * 
 */
class  VCN_Model_Socxcip extends VCN_Model_Base_Socxcip {
	
	protected $valid = array('SOCCODE','SXC.CIPCODE', 'ONETCODE_NEW');
	
	public function countSocxcip($params, $fields=false) 
	{
		// Crosswalk stuff
		if (array_key_exists('onetcode', $params)) { $params['onetcode_new'] = $params['onetcode']; }
		// Duplicate filter
		if (array_key_exists('cipcode', $params)) { $params['sxc.cipcode'] = $params['cipcode']; }
		...
		$stmt->from('VCN_Model_Socxcip SXC ')
   			->leftJoin('SXC.SocXOnet sxo')
   			->leftJoin('sxo.OnetXWalk bcw');
   					
        if ($query['where']) { foreach ($query['where'] AS $where) { $stmt->addwhere($where); } }
  
 	    $data['count'] = $stmt->fetchArray()->count();
	    
		// Crosswalk stuff
		if (array_key_exists('onetcode_new', $params)) { unset($params['onetcode_new']); }
		
		// Duplicate filter
 		if (array_key_exists('sxc.cipcode', $params)) unset($params['sxc.cipcode']);
	...
	public function listSocxcip($params, $fields=false) {
		// Crosswalk stuff
		if (array_key_exists('onetcode', $params)) { $params['onetcode_new'] = $params['onetcode']; }
 		// Duplicate filter
		if (array_key_exists('cipcode', $params)) { $params['sxc.cipcode'] = $params['cipcode']; }
		...
		// Crosswalk stuff
		if (array_key_exists('onetcode_new', $params)) { $params['onetcode'] = $params['onetcode_new']; }
		// Duplicate filter
 		if (array_key_exists('sxc.cipcode', $params)) unset($params['sxc.cipcode']);
	...
	public function getSocxcip($params) {
    	// Crosswalk stuff
		if (array_key_exists('onetcode', $params)) { $params['onetcode_new'] = $params['onetcode']; }
		// Duplicate filter
		if (array_key_exists('cipcode', $params)) { $params['sxc.cipcode'] = $params['cipcode']; }
