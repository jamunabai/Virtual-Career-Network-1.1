 ...
 * 
 */
class  VCN_Model_ScalesReference extends VCN_Model_Base_ScalesReference {
	protected $valid = array('SCALEID');
	
	public function countScalesReferences($params) 
	{
	...
	public function listScalesReferences($params) {
		$query = $this->parseParams($params, $this->valid);
        $stmt  = $this->getTable()->createQuery();
		$select= $this->parseFields($stmt, $fields);
 		if ($select) { foreach ($select AS $field) { $stmt->addselect($field); } }
	...
	public function getScalesReference($params) {
		$query = $this->parseParams($params, $this->valid);
 		$stmt  = $this->getTable()->createQuery();
  		$required = array('scaleid');
