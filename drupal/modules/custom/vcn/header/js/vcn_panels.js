...
*/ 
 ...
 function not_logged_in(usersession,what) { 
     if (usersession!='U') { 
		 ModalPopups.YesNoCancel("idConfirm1",  
		 "<div style='text-align:center; margin-left:55px;'>"+what+"</div>",  
		 "<div style='text-align:center; margin-bottom:10px;'><b>To make it permanent, please login if you have an account, <br /> or register to create a new account. Otherwise continue.</b></div>",   
		 {  
		 titleBackColor: "#808080",  
		 titleFontColor: "white",  
		 popupBackColor: "#808080",  
		 popupFontColor: "white",  
		 footerBackColor: "#808080",  
 ...
 function red_error_box(what) { 
		what2='';
		if (what==4) {
			what='Only 4 careers are allowed. Please remove one of the';
			what2='existing saved careers before adding another.';
		}
		else if (what=="delete") {
			what='Please choose another target career before deleting this.';
			what2='You will need to go to next activity (Choose Your Target Career) to do that.';
		}
		else if (what=="cantdelete") {
			what='Please choose another target career before deleting this.';
			what2='You will need to go to choose a career (Choose Your Target Career) to do that.';
		}
		 ModalPopups.Custom("idConfirm1",  
		 "<div style='text-align:center; margin-left:25px;'>"+what+"</div>",  
		 "<div style='text-align:center; margin-bottom:10px;font-weight: bold; margin-top: -10px;'>"+what2+"</div>",   
		 {  
			 titleBackColor: "#A71E29",  
			 titleFontColor: "white",  
			 popupBackColor: "#A71E29",  
			 popupFontColor: "white",  
			 footerBackColor: "#A71E29",  
			 footerFontColor: "white",
			 fontSize: "10pt",
			 fontType: "bold",
			 width: 452,
...
function ModalPopupsConfirmYes() {  
    ...
    //window.open('/drupal/user/register','_self'); 
	window.open('https://'+tempurlparts[2]+'/'+tempurlparts[3]+'/user/register?href='+encodeURIComponent(document.URL),'_self'); 
    ModalPopups.Close("idConfirm1");  
}  
function ModalPopupsConfirmNo() {  
	var tempurl = document.location.href;
	var tempurlparts = tempurl.split('/');
    //window.open('/drupal/user/login','_self');
	window.open('https://'+tempurlparts[2]+'/'+tempurlparts[3]+'/user?href='+encodeURIComponent(document.URL),'_self'); 
    ModalPopups.Close("idConfirm1");  
}  
function ModalPopupsCancel2() {  
    ModalPopups.Close("idConfirm1");  
}  

function comment(url,theemail,theid,datetime) {  
	url = window.location.protocol+"//"+url;

    ModalPopups.Custom("idCustom1",  
        "Enter comment",  
        "<form action='javascript:return false;' method='post'><table width='500' cellpadding='3' cellspacing='0'><tr><td>Email:</td><td><label for='emailcom'><input type='text' name='emailcom' value='"+theemail+"' id='emailcom' /></label></td></tr><tr><td width='75'>Subject:</td><td><label for='subject'><input type='text' name='subject' id='subject' /></label></td><tr><td width='75'>Comment:</td><td><label for='comment'><textarea rows='10' cols='50' name='comment' id='comment'></textarea></label></td></tr><tr><td width='75'></td><td align='right'></td></tr></table><input type='hidden' name='url' id='url' value='"+url+"' /><label for='bycom'><input type='hidden' name='bycom' id='bycom' value='"+theid+"' /></label><label for='datetime'><input type='hidden' name='datetime' id='datetime' value='"+datetime+"' /></label><div style='display:none;' id='loadherec'></div></form>",   
        {  
            width: 550,  
            buttons: "ok,cancel",  
            okButtonText: "Submit",  
...
function ModalPopupsCustom1Save() {  
    var url = ModalPopups.GetCustomControl("url").value;   
	var email = ModalPopups.GetCustomControl("emailcom").value; 
	var id = ModalPopups.GetCustomControl("bycom").value; 
	var datetime = ModalPopups.GetCustomControl("datetime").value; 
	var subject = ModalPopups.GetCustomControl("subject").value; 
	var comment = ModalPopups.GetCustomControl("comment").value; 
	comment = comment.replace(/ /g, "~");
	subject = subject.replace(/ /g, "~");

        //alert(url+' '+email+' '+id+' '+datetime);  
		//alert('/careerladder/comment.php?url='+url+'&email='+email+'&subject='+subject+'&comment='+comment+'&by='+id+'&datetime='+datetime);
		
		if (!comment)
			alert('Please enter a comment.');
		else		
		   $('#loadherec').load('/careerladder/commentdiffquery.php?url='+url+'&email='+email+'&subject='+subject+'&comment='+comment+'&by='+id+'&datetime='+datetime, function() {
...
var ModalPopups = {
    ...
    YesNoCancel: function(id, title, question, parameters) {
        ...
        if (document.getElementById("idConfirm1_cancelButton")) {
        	document.getElementById("idConfirm1_cancelButton").focus();
        }
    ...
    Custom: function(id, title, contents, parameters) {
        ...
        if (document.getElementById("idConfirm1_okButton")) {
        	document.getElementById("idConfirm1_okButton").focus();
    	}
