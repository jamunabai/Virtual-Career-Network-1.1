...
Drupal.behaviors.cmaNotebookRemoveCourseAjax = function(context) {

  // Bind an AJAX callback to our link
  $('.course-remove-button').click(function(event) {
    // Get the URL without the query string - this is
    // so that we can distinguish between GET and POST
    // requests.
    var cmaNotebookRemoveCourseUrl = $(this).attr('href').split('?');
    var cmaid = $(this).attr('careercmaid');

    var cmaCourseId = '#course-' + cmaNotebookRemoveCourseUrl[0].split('/').pop().replace('.','-');
    ...
    $.ajax({
      type: "POST",
      url: cmaNotebookRemoveCourseUrl[0],
      dataType: "html",
      success: function (html) {
        $(cmaCourseId).empty().remove();
          if (html[0] == ':') {
			  ...
			  not_logged_in('S','Course Removed from your Wish List.');
              if (answer) {
...
// applied to any new occurances of #example-ajax
// created on the fly by other scripts.
Drupal.behaviors.cmaNotebookTargetCourseAjax = function(context) {

  // Bind an AJAX callback to our link
  $('.course-target-button').click(function(event) {
    // Get the URL without the query string - this is
    // so that we can distinguish between GET and POST
    // requests.
    var cmaNotebookTargetCourseUrl = $(this).attr('href').split('?');
    var cmaCourseId = '#course-' + cmaNotebookTargetCourseUrl[0].split('/').pop().replace('.','-');
    var target_course_id = '#target_course_'+cmaNotebookTargetCourseUrl[0].split('/').pop().replace('.','-');
    ...
    $.ajax({
      type: "POST",
      url: cmaNotebookTargetCourseUrl[0],
      dataType: "html",
      success: function (html) {
    	$(cmaCourseId).parent().prepend($(cmaCourseId));
    	//alert(cmaCourseId);
       // $('#vcn-cma-notebook-course-inner').prepend(document.getElementById(cmaCourseId));
          if (html[0] == ':') {
              //var want2login = html.substring(1) + '\n\nYou do not appear to be logged in. Would you like to log in now?';
             // var answer = confirm(want2login);
			 not_logged_in('S','Course targeted.');
              if (answer) {
                  //window.location = base_path+"user/login";
              	//To make sure the login is through /user not through /user/login
              	  window.location = base_path+"user";
          ...
          $('.target_course').empty();         
          $(target_course_id).text('(This is the Targeted Course)');        	  
          html = html.split('###');
          alert(html[1]);
      },
      error: function (xmlhttp) {
